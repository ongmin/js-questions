...
DerivedModel.prototype = Object.getPrototypeOf(new BaseModel());
...
BaseModel.prototype.getNumber = function () { return 2 }
...
Model.test = Model.prototype.test.bind(k);
...
onClick () {
  var addOne = this.state.times_clicked + 1
  this.setState({ times_clicked: addOne })
}
...
class RootComponent extends React.Component {
  ...
  render () {
    const { childValues } = this.props
    return childValues.map(({name, value}, index) => {
      return (
        <ChildComponent name={name} value={value} key={index}/>
      )
    })
  }
...
// -----------------------------------------------------------------------------------------
  ...
  */
  ...
  class RootComponent extends React.Component {
    ...
    onChildClick = (parent) => (num) => {
      var newNumber = this.state.number + num
      console.log(newNumber)

      // I assumed over here that you want to continually increment the state number
  ...
  render () {
    return (
      <div className="child">
        {
          this.props.name ?
          <div className="name-display">{this.props.name}</div>
          : null
        }
        <div className={"value-display " + (this.props.value < 10 ? 'low' : 'high')}>
          {this.props.value}
        </div>
      </div>
    )
